#!/usr/bin/env python3
"""
Developer Skill Analyzer Console - Personal Analysis Focus
...
class SkillConsole:
    ...
    def menu(self):
        ...
        print("1. 🏠 Analyze YOUR GitHub Account")
        print("2. 🤝 Find ALL Your Collaborators & Their Skills")
        print("3. 📊 Analyze Specific Developer")
        print("4. ⚙️ Check Setup")
        print("0. Exit")
        print("-"*60)
    ...
    def analyze_your_github(self):
        ...
        try:
            ...
            # Analyze repositories and skills
            print(f"\n📁 YOUR TOP REPOSITORIES:")
            repos = list(user.get_repos(type='owner', sort='updated'))[:12]
            ...
            for i, repo in enumerate(repos, 1):
                print(f"{i:2d}. {repo.name:25} (⭐{repo.stargazers_count:4d}, 🍴{repo.forks_count:3d})")
                total_stars += repo.stargazers_count
                
                # Get languages
                try:
                    languages = repo.get_languages()
                    repo_data.append({'name': repo.name, 'languages': languages})
            ...
            if languages_used:
                print(f"\n🎯 YOUR PROGRAMMING COMPETENCIES:")
                sorted_langs = sorted(languages_used.items(), key=lambda x: x[1], reverse=True)[:10]
                
                for lang, bytes_count in sorted_langs:
                    # Create language-specific data for competency assessment
                    lang_specific = {lang: bytes_count}
                    level_name, level_emoji, score = self.assess_competency_level(lang_specific, repo_data)
                    total_bytes = sum(languages_used.values())
                    percentage = (bytes_count / total_bytes) * 100
                    print(f"   {level_emoji} {lang:15}: {level_name:12} ({percentage:5.1f}% usage, score: {score:.0f})")
            
            # Analyze your frameworks and tools
            frameworks = self.detect_frameworks_and_tools(repos)
            if frameworks:
                print(f"\n🔧 YOUR FRAMEWORKS & TOOLS:")
                sorted_frameworks = sorted(frameworks.items(), key=lambda x: x[1], reverse=True)[:8]
                for fw, count in sorted_frameworks:
                    if count >= 1:  # Show all detected frameworks
                        level_emoji = "🏆" if count >= 5 else "🥇" if count >= 4 else "🥈" if count >= 2 else "🥉"
                        competency = "Expert" if count >= 5 else "Advanced" if count >= 4 else "Intermediate" if count >= 2 else "Beginner"
                        print(f"   {level_emoji} {fw:18}: {competency:12} (Used in {count} repositories)")
        ...
        try:
            ...
            for i, collaborator in enumerate(top_collaborators, 1):
                ...
                try:
                    ...
                    # Analyze their programming languages and competencies
                    try:
                        collab_repos = list(collab_user.get_repos(type='owner', sort='updated'))[:8]
                        collab_languages = {}
                        repo_data = []
                        ...
                        for repo in collab_repos:
                            try:
                                ...
                                repo_data.append({'name': repo.name, 'languages': repo_langs})
                        ...
                        if collab_languages:
                            print(f"🎯 PROGRAMMING COMPETENCIES:")
                            ...
                            for lang, bytes_count in top_langs:
                                ...
                                percentage = (bytes_count / total_bytes) * 100
                                print(f"   {level_emoji} {lang:12}: {level_name:12} ({percentage:4.1f}% usage, score: {score:.0f})")
                        
                        # Detect frameworks and tools
                        frameworks = self.detect_frameworks_and_tools(collab_repos)
                        if frameworks:
                            print(f"🔧 FRAMEWORKS & TOOLS:")
                            sorted_frameworks = sorted(frameworks.items(), key=lambda x: x[1], reverse=True)[:4]
                            for fw, count in sorted_frameworks:
                                if count >= 2:  # Only show if used in multiple repos
                                    level_emoji = "🏆" if count >= 4 else "🥇" if count >= 3 else "🥈"
                                    print(f"   {level_emoji} {fw:15}: Used in {count} repositories")
                        
                    except Exception as e:
                        print(f"⚠️ Could not analyze competencies: {e}")
    ...
    def analyze_specific_developer(self):
        ...
        try:
            ...
            # Analyze their repositories and skills
            ...
            if repos:
                ...
                if frameworks:
                    print(f"\n🔧 FRAMEWORKS & TOOLS:")
                    sorted_frameworks = sorted(frameworks.items(), key=lambda x: x[1], reverse=True)[:6]
                    for fw, count in sorted_frameworks:
                        if count >= 1:  # Show all detected frameworks
                            level_emoji = "🏆" if count >= 4 else "🥇" if count >= 3 else "🥈" if count >= 2 else "🥉"
                            competency = "Expert" if count >= 4 else "Advanced" if count >= 3 else "Intermediate" if count >= 2 else "Beginner"
                            print(f"   {level_emoji} {fw:18}: {competency:12} (Used in {count} repos)")
    ...
    async def run(self):
        """Main console loop."""
        print("🚀 Developer Skill Analyzer Console - Personal Analysis Focus")
